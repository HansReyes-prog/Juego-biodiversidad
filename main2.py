...
def pantalla_inicio():
    ...
    dibujar_texto("Biodiversidad", font_titulo, constantes.BLANCO,
                  constantes.ANCHO_VENTANA / 3 - 200,
                  constantes.ALTO_VENTANA / 3 - 200)

    pygame.draw.rect(ventana, constantes.AMARILLO, boton_jugar)
    pygame.draw.rect(ventana, constantes.ROJO, boton_salir)
    ventana.blit(texto_boton_jugar, (boton_jugar.x + 10, boton_jugar.y + 2))
    ventana.blit(texto_boton_salir, (boton_salir.x + 10, boton_salir.y + 2))
...
while run == True:
    if mostrar_inicio:
        ...
        dibujar_texto(f"Score: {jugador.score}", font, (255,255,0), 400,10)
        #nivel
        dibujar_texto(f"Nivel: " + str(nivel), font, constantes.BLANCO, constantes.ANCHO_VENTANA / 3, 10)
